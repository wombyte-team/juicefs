...
public class JuiceFileSystemImpl extends FileSystem {
    ...
    static String loadVersion() {
        try (
            InputStream in =
                JuiceFileSystemImpl.class.getClassLoader()
                    .getResourceAsStream("juicefs-ver.properties")
        ) {
    ...
    private ConsistentHash<String> hash = new ConsistentHash<>(
        1,
    ...
    private static final String GROUPNAME_GID_USERNAMES_PATTERN =
  ...
  */
    ...
    public void initialize(URI uri, Configuration conf) throws IOException {
        ...
        blocksize = conf.getLongBytes(
            "juicefs.block.size",
            conf.getLongBytes("dfs.blocksize", 128 << 20)
        );
        minBufferSize = conf.getInt("juicefs.min-buffer-size", 128 << 10);
        cacheReplica = Integer.parseInt(getConf(conf, "cache-replica", "1"));
        fileChecksumEnabled = Boolean.parseBoolean(
            getConf(conf, "file.checksum", "false")
        );
        ...
        String supergroup = getConf(
            conf,
            "supergroup",
            conf.get("dfs.permissions.superusergroup", "supergroup")
        );
        ...
        obj.put(
            "readOnly",
            Boolean.valueOf(getConf(conf, "read-only", "false"))
        );
        obj.put(
            "noSession",
            Boolean.valueOf(getConf(conf, "no-session", "false"))
        );
        ...
        obj.put(
            "backupSkipTrash",
            Boolean.valueOf(getConf(conf, "backup-skip-trash", "false"))
        );
        ...
        obj.put(
            "cacheFullBlock",
            Boolean.valueOf(getConf(conf, "cache-full-block", "true"))
        );
        obj.put(
            "cacheChecksum",
            getConf(conf, "verify-cache-checksum", "full")
        );
        obj.put("cacheEviction", getConf(conf, "cache-eviction", "2-random"));
        obj.put(
            "cacheScanInterval",
            getConf(conf, "cache-scan-interval", "300")
        );
        obj.put("cacheExpire", getConf(conf, "cache-expire", "0"));
        obj.put(
            "autoCreate",
            Boolean.valueOf(getConf(conf, "auto-create-cache-dir", "true"))
        );
        obj.put(
            "maxUploads",
            Integer.valueOf(getConf(conf, "max-uploads", "20"))
        );
        obj.put(
            "maxDeletes",
            Integer.valueOf(getConf(conf, "max-deletes", "10"))
        );
        obj.put(
            "skipDirNlink",
            Integer.valueOf(getConf(conf, "skip-dir-nlink", "20"))
        );
        ...
        obj.put(
            "ioRetries",
            Integer.valueOf(getConf(conf, "io-retries", "10"))
        );
        obj.put(
            "getTimeout",
            getConf(conf, "get-timeout", getConf(conf, "object-timeout", "5"))
        );
        obj.put(
            "putTimeout",
            getConf(conf, "put-timeout", getConf(conf, "object-timeout", "60"))
        );
        ...
        obj.put(
            "fastResolve",
            Boolean.valueOf(getConf(conf, "fast-resolve", "true"))
        );
        obj.put(
            "noUsageReport",
            Boolean.valueOf(getConf(conf, "no-usage-report", "false"))
        );
        ...
        if (withStreamCapability) {
            try {
                constructor = Class.forName(
                    "io.juicefs.JuiceFileSystemImpl$BufferedFSOutputStreamWithStreamCapabilities"
                ).getConstructor(
                    OutputStream.class,
                    Integer.TYPE,
                    String.class
        ...
        boolean hasAclMtd = ReflectionUtil.hasMethod(
            FileStatus.class.getName(),
            "hasAcl",
            (String[]) null
        );
        if (hasAclMtd) {
            fileStatusConstructor = ReflectionUtil.getConstructor(
                FileStatus.class,
                long.class,
                boolean.class,
                int.class,
        ...
        try {
            ...
            if (uri.getScheme() == null) {
                ...
                if (
                    ...
                    (uri.getScheme().equals(defaultUri.getScheme()))
                ) {
                    uri = defaultUri;
                }
            }
            ...
            if (
                ...
                status.getModificationTime() ==
                    lastStatus.getModificationTime() &&
                status.getLen() == lastStatus.getLen()
            ) {
                return null;
            }
            FSDataInputStream in = fs.open(path);
            String res = new BufferedReader(new InputStreamReader(in))
                .lines()
    ...
    private void updateUidAndGrouping(String uidFile, String groupFile)
        ...
        String grouping = parseUidAndGrouping(
    ...
    private void initializeStorageIds(Configuration conf) throws IOException {
        ...
        storageIds = new String[java.lang.Runtime.getRuntime()
            .availableProcessors() *
        vdiskPerCpu];
    ...
    public Path getHomeDirectory() {
        return makeQualified(
            new Path(homeDirPrefix + "/" + ugi.getShortUserName())
        );
    }
    ...
    private static void initStubLoader() {
        ...
        // try try try ...
        while (
            StubLoader.getFailureCause() != null &&
            (System.currentTimeMillis() - start) < loadMaxTime * 1000
        ) {
    ...
    public static Libjfs loadLibrary() {
        ...
        LibraryLoader<Libjfs> libjfsLibraryLoader = LibraryLoader.create(
            Libjfs.class
        );
        ...
        long soTime;
        URL location =
            JuiceFileSystemImpl.class.getProtectionDomain()
                .getCodeSource()
                .getLocation();
        if (location == null) {
            // jar may changed
        ...
        try {
            ...
            if (
                ...
                (con instanceof JarURLConnection)
            ) {
                ...
                if (Files.isDirectory(Paths.get(locationUri))) { // for debug: sdk/java/target/classes
                    ...
                    ins = JuiceFileSystemImpl.class.getClassLoader()
    ...
    private static Libjfs loadExistLib(
        LibraryLoader<Libjfs> libjfsLibraryLoader,
        File dir,
        String name,
        File libFile
    ) {
        File currentUserLib = new File(
            dir,
            System.getProperty("user.name") + "-" + name
        );
        ...
        try {
            ...
            if (urls != null) {
                ...
                for (String newNode : newNodes) {
                    try {
                        newCachedHosts.put(
                            InetAddress.getByName(newNode).getHostAddress(),
                            newNode
                        );
    ...
    private List<String> discoverNodes(String urls) {
        ...
        NodesFetcher fetcher = NodesFetcherBuilder.buildFetcher(
            urls,
            name,
            this
        );
    ...
    private BlockLocation makeLocation(long code, long start, long len) {
        ...
        String host = cachedHosts.getOrDefault(
            hash.get(code + "-" + index),
            "localhost"
        );
        ns[0] = host + ":50010";
        ...
        blockLocation = new BlockLocation(
            ns,
            hs,
            null,
            null,
    ...
    public BlockLocation[] getFileBlockLocations(
        ...
        if (cacheReplica <= 0) {
            ...
            return new BlockLocation[] {
                new BlockLocation(name, host, 0L, file.getLen()),
            };
        }
        ...
        if (indx > 1 && locs[indx - 1].getLength() < blocksize / 10) {
            locs[indx - 2].setLength(
                    locs[indx - 2].getLength() + locs[indx - 1].getLength()
                );
            indx--;
        }
    ...
    class FileInputStream extends FSInputStream implements ByteBufferReadable {
        ...
        public FileInputStream(Path f, int fd, int size, long fileLen)
            throws IOException {
            path = f;
            this.fd = fd;
        ...
        public synchronized long getPos() throws IOException {
            if (buf == null) throw new IOException("stream was closed");
        ...
        public synchronized int read() throws IOException {
            if (buf == null) throw new IOException("stream was closed");
            if (!buf.hasRemaining() && !refill()) return -1; // EOF
        ...
        private synchronized int read(long pos, ByteBuffer b)
            throws IOException {
            if (pos < 0) throw new EOFException("position is negative");
            if (!b.hasRemaining()) return 0;
            int got;
            ...
            if (got == EINVAL) throw new IOException("stream was closed");
            if (got < 0) throw error(got, path);
            if (got == 0) return -1;
    ...
    public FSDataInputStream open(Path f, int bufferSize) throws IOException {
        ...
        return new FSDataInputStream(
            new FileInputStream(f, fd, checkBufferSize(bufferSize), len)
        );
    }

    @Override
    public void access(Path path, FsAction mode) throws IOException {
        int r = lib.jfs_access(
            Thread.currentThread().getId(),
            handle,
            normalizePath(path),
            mode.ordinal()
    ...
    class FSOutputStream extends OutputStream {
        ...
        public void hflush() throws IOException {
            ...
            if (r == EINVAL) throw new IOException("stream was closed");
            if (r < 0) throw error(r, path);
        }
        ...
        public void write(byte[] b, int off, int len) throws IOException {
            ...
            int done = lib.jfs_write(
                Thread.currentThread().getId(),
                fd,
                ByteBuffer.wrap(b, off, len),
                len
            );
            if (done == EINVAL) throw new IOException("stream was closed");
            if (done < 0) throw error(done, path);
            if (done < len) {
        ...
        public void write(int b) throws IOException {
            ...
            if (done == EINVAL) throw new IOException("stream was closed");
            if (done < 0) throw error(done, path);
            if (done < 1) throw new IOException("write");
        }
    }

    static class BufferedFSOutputStream
        ...
        public synchronized void hflush() throws IOException {
            ...
            if (hflushMethod.equals("writeback")) {
                ...
                hflushMethod.equals("sync") || hflushMethod.equals("fsync")
            ) {
    ...
    static class BufferedFSOutputStreamWithStreamCapabilities
        extends BufferedFSOutputStream
        ...
        public BufferedFSOutputStreamWithStreamCapabilities(
            OutputStream out,
            int size,
            String hflushMethod
        ) {
        ...
        public boolean hasCapability(String capability) {
            return (
                capability.equalsIgnoreCase("hsync") ||
        ...
        if (withStreamCapability) {
            try {
                return new FSDataOutputStream(
                    (OutputStream) constructor.newInstance(
                        out,
    ...
    public FileChecksum getFileChecksum(Path f, long length)
        throws IOException {
        statistics.incrementReadOps(1);
        if (!fileChecksumEnabled) return null;
        String combineMode = getConf()
            .get("dfs.checksum.combine.mode", "MD5MD5CRC");
        if (!combineMode.equals("MD5MD5CRC")) return null;
        DataChecksum.Type ctype = DataChecksum.Type.valueOf(
            getConf().get("dfs.checksum.type", "CRC32C")
        );
        if (ctype.size != 4) return null;
        ...
        while (got < length && !eof) {
            ...
            if (crcBuf.getLength() > 0) {
                MD5Hash blockMd5 = MD5Hash.digest(
                    crcBuf.getData(),
                    0,
                    crcBuf.getLength()
                );
    ...
    public boolean rename(Path src, Path dst) throws IOException {
        ...
        if (r == ENOENT || r == EEXIST) return false;
        if (r == EACCESS) {
            this.access(
                    makeQualified(src).getParent(),
                    FsAction.WRITE.or(FsAction.EXECUTE)
                );
            this.access(
                    makeQualified(dst).getParent(),
                    FsAction.WRITE.or(FsAction.EXECUTE)
                );
        }
        if (r < 0) throw error(r, src);
        return true;
    }

    @Override
    public boolean truncate(Path f, long newLength) throws IOException {
        int r = lib.jfs_truncate(
            Thread.currentThread().getId(),
            handle,
            normalizePath(f),
            newLength
        );
        if (r < 0) throw error(r, f);
        return true;
    }
    ...
    public boolean delete(Path p, boolean recursive) throws IOException {
        statistics.incrementWriteOps(1);
        if (recursive) return rmr(p);
        int r = lib.jfs_delete(
            Thread.currentThread().getId(),
            handle,
            normalizePath(p)
        );
    ...
    private FileStatus newFileStatus(
        Path p,
        Pointer buf,
        int size,
        boolean readlink
        ...
        if (fileStatusConstructor == null) {
            ...
            try {
                return fileStatusConstructor.newInstance(
                    length,
                    isdir,
                    1,
                    blocksize,
    ...
    private FileStatus getFileStatusInternal(final Path f, boolean dereference)
        throws IOException {
        String path = normalizePath(f);
    ...
    private FileStatus getFileStatusInternalNoException(final Path f)
        throws IOException {
        String path = normalizePath(f);
        Pointer buf = Memory.allocate(Runtime.getRuntime(lib), 130);
        int r = lib.jfs_lstat1(
            Thread.currentThread().getId(),
            handle,
            path,
            buf
    ...
    public void setXAttr(
        ...
        if (
            flag.contains(XAttrSetFlag.CREATE) &&
            flag.contains(XAttrSetFlag.REPLACE)
        ) {
            mode = 0;
    ...
    public byte[] getXAttr(Path path, String name) throws IOException {
        ...
        if (r == ENOATTR || r == ENODATA) return null; // attr not found
        if (r < 0) throw error(r, path);
    ...
    public void setAcl(Path path, List<AclEntry> aclSpec) throws IOException {
        List<AclEntry> existingEntries = getAllAclEntries(path);
        List<AclEntry> newAcl = AclTransformation.replaceAclEntries(
            existingEntries,
            aclSpec
        );
        setAclInternal(path, newAcl);
    ...
    private void setAclInternal(Path path, List<AclEntry> aclSpec)
        throws IOException {
        List<AclEntry> aclEntries = AclTransformation.buildAndValidateAcl(
            Lists.newArrayList(aclSpec)
        );
    ...
    private void removeAclInternal(Path path, AclEntryScope scope)
        throws IOException {
    ...
    private void setAclInternal(
        Path path,
        AclEntryScope scope,
        List<AclEntry> aclSpec
    ) throws IOException {
        if (aclSpec.size() == 0) return;
    ...
    private List<AclEntry> getAclEntries(Path path, AclEntryScope scope)
        throws IOException {
        int bufsize = 1024;
        int r;
        Pointer buf;
        ...
        if (userperm != -1) {
            entries.add(
                builder
                    ...
                    .setPermission(FsAction.values()[userperm])
                    .build()
            );
        }
        if (groupperm != -1) {
            entries.add(
                builder
                    ...
                    .setPermission(FsAction.values()[groupperm])
                    .build()
            );
        }
        if (otherperm != -1) {
            entries.add(
                builder
                    ...
                    .setPermission(FsAction.values()[otherperm])
                    .build()
            );
        }
        if (maskperm != -1) {
            entries.add(
                builder
                    ...
                    .setPermission(FsAction.values()[maskperm])
                    .build()
            );
        }
        ...
        for (int i = 0; i < namedusers + namedgroups; i++) {
            ...
            entries.add(
                builder
                    .setType(
                        i < namedusers ? AclEntryType.USER : AclEntryType.GROUP
                    )
                    .setName(name)
    ...
    private List<AclEntry> getAclEntries(Path path) throws IOException {
        ...
        List<AclEntry> accessEntries = getAclEntries(
            path,
    ...
    public AclStatus getAclStatus(Path path) throws IOException {
        ...
        AclStatus.Builder builder = new AclStatus.Builder()
            .owner(st.getOwner())
            .group(st.getGroup())
            .stickyBit(st.getPermission().getStickyBit())
            .addEntries(entries);
        try {
            Class<AclStatus.Builder> ab = AclStatus.Builder.class;
            Method abm = ab.getDeclaredMethod(
                "setPermission",
                FsPermission.class
            );
